...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var gameoverY = 0;
  var endingSound = true;
  var mainLoop = function mainLoop() {
    var mainLoopId = void 0;
    if (!game.pause) {
      game.render();
    }
    ...
    if (game.gameover && gameoverY > -1500) {
      ...
      gameover.onload = function () {
        return game.canvas.drawImage(gameover, 0, gameoverY, 700, 2000);
      };
      if (endingSound) {
        game.sounds.music.levelOne.pause();
        setTimeout(function () {
          return game.sounds.music.credits.play();
        }, 2000);
        endingSound = false;
      }
    }
...
function instructions(game) {
  ...
  game.gameCanvas.addEventListener("click", function () {
    game.pause = false;
    ...
    value: function render() {
      ...
      if (!this.gameover) {
        ...
        this.enemies.forEach(function (enemy) {
          enemy.X += -_this.player.Velocity_X;
          enemy.update();
        });
    ...
    value: function update() {
      ...
      if (this.Y > 540) {
        ...
        setTimeout(function () {
          ...
          _this2.gravity = 500, _this2.weight = 2, _this2.addKeyboardEvents();
        }, 1000);
      }
...
var LevelOne = function () {
  ...
  _createClass(LevelOne, [{
    ...
    value: function getBoss() {
      return new _boss2.default("assets/boss.png", 6740, 390, 282, 262, 15000, this.game);
    }
  }, {
    key: 'drawLevel',
      ...
      if (!this.dead) {
        this.game.player.bullets.forEach(function (bullet) {
          if (_this2.isColliding(bullet)) {
            ...
            _this2.game.score += 500;
            _this2.health -= 100;
            _this2.game.player.Velocity_X = -4.1;
            _this2.game.boss.X += -_this2.game.player.Velocity_X;;
            _this2.game.platforms.forEach(function (platform) {
