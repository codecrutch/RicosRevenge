...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__objects_enemy__ = __webpack_require__(17);
...
class LevelOne {
  ...
  generateBoundary(startPos, heightPos, platformWidth, platformHeight = 100, img = "") {
    this.boundaries.push(new __WEBPACK_IMPORTED_MODULE_0__objects_platform__["a" /* default */](img, startPos += 48, heightPos, platformWidth, platformHeight))
  };

  getEnemies() {
    while (this.enemies.length < this.maxEnemies) {
      this.enemies.push(new __WEBPACK_IMPORTED_MODULE_1__objects_enemy__["a" /* default */](this.game))
    }
    return this.enemies;
  }
...
class Player extends __WEBPACK_IMPORTED_MODULE_0__baseObject__["a" /* default */] {
  ...
  update() {
    ...
    if (this.Y > 540) {
      ...
      setTimeout(() => {
        this.Y = -100,
          ...
          this.game.boundaries = this.game.level.getBoundaries(),
          this.gravity = 500,
          this.weight = 2,
    ...
    this.game.boundaries.forEach(boundary => {
      if (this.isColliding(boundary)) {
...
class Game {
  constructor() {
    ...
    this.level = new __WEBPACK_IMPORTED_MODULE_1__levels_one__["a" /* default */](this);
  ...
  render() {
    ...
    this.enemies.forEach(enemy => {enemy.X += -this.player.Velocity_X; enemy.update();});
    this.background.X += -this.player.Velocity_X;
    this.player.update();
    // Render all images onto canvas
    __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_3__util_levelRender__["a" /* default */])(this);
  }
}
...
class Enemy extends __WEBPACK_IMPORTED_MODULE_0__baseObject__["a" /* default */] {
  constructor(game) {
    super("assets/bat.png", Math.random() * 300, Math.random() * 1000, 10, 10)
    this.game = game;
    this.weight = 0;
    this.gravity = 0;
    this.direction = 'right';
