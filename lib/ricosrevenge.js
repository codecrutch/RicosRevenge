...
var Game = function () {
  function Game() {
    ...
    this.background = new _background2.default("assets/background.png", 0, 0, 1000, 1000);
  }
  ...
  _createClass(Game, [{
    ...
    value: function render() {
      ...
      this.platforms.forEach(function (platform) {
        return platform.X += -_this.player.Velocity_X;
      });
      this.boundaries.forEach(function (boundary) {
        return boundary.X += -_this.player.Velocity_X;
      });
      this.enemies.forEach(function (enemy) {
        enemy.X += -_this.player.Velocity_X;enemy.update();
      });
  ...
  _createClass(Player, [{
    ...
    value: function update() {
      ...
      if (this.Y > 540) {
        ...
        setTimeout(function () {
          _this2.Y = -100, _this2.game.background.X = 0, _this2.game.platforms = _this2.game.level.getPlatforms(), _this2.game.boundaries = _this2.game.level.getBoundaries(), _this2.gravity = 500, _this2.weight = 2, _this2.addKeyboardEvents();
        }, 1000);
      }
      ...
      this.game.boundaries.forEach(function (boundary) {
        if (_this2.isColliding(boundary)) {
          // console.log("collision detected");
          _this2.game.platforms = _this2.game.level.getPlatforms(), _this2.game.boundaries = _this2.game.level.getBoundaries(), _this2.game.background.X = 0;
          _this2.X = 200;
          _this2.Velocity_X = 0;
        }
      });
    ...
    value: function pauseGame(e) {
      ...
      if (e.keyCode === 112) {
        ...
        if (muteButton.innerHTML === 'Mute Sound') {
          mute = function mute() {
            return _this4.game.sounds.mute();
          };
          unmute = function unmute() {
            return _this4.game.sounds.unmute();
          };
        }
      ...
      switch (e.keyCode) {
        case 13:
          if (e.type === "keydown") {
            this.shootBullet();
            setTimeout(function () {
              return _this5.isShooting = false;
            }, 100);
          } else {
...
var LevelOne = function () {
  ...
  _createClass(LevelOne, [{
    ...
    value: function generateBoundary(startPos, heightPos, platformWidth) {
      var platformHeight = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 100;
      var img = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : "";

      this.boundaries.push(new _platform2.default(img, startPos += 48, heightPos, platformWidth, platformHeight));
    }
  }, {
    key: 'getEnemies',
    ...
    value: function drawBoundaries() {
      this.generateBoundary(70, 210, 75, 100); // start boundary
      this.generateBoundary(6825, 0, 75, 10000); // end boundary
    }
  }, {
    key: 'drawLevel',
...
var Enemy = function (_BaseObject) {
  ...
  function Enemy(game) {
    ...
    var _this = _possibleConstructorReturn(this, (Enemy.__proto__ || Object.getPrototypeOf(Enemy)).call(this, "assets/bat.png", Math.random() * 1000, Math.random() * 1000, 10, 20));

    _this.game = game;
    _this.weight = 0;
    _this.gravity = 0;
    _this.direction = 'right';
    _this.flyingFrameCount = 0;
  ...
  _createClass(Enemy, [{
    ...
    value: function draw() {
      ...
      var sprite = this.selectSprite();
      this.game.canvas.drawImage(this.Sprite, sprite[0], sprite[1], sprite[2], sprite[3], this.X, this.Y, 32, 32);
      if (this.direction === "left") {
        this.game.canvas.scale(-1, 1);
      }
    }
...
var levelRender = function levelRender(game) {
  ...
  boundaries.forEach(function (boundary) {
    return canvas.drawImage(boundary.Sprite, boundary.X, boundary.Y, boundary.width + 50, boundary.height);
  });
